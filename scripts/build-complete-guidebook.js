...
<main>
  ...
  <section id="progression"><h2>12. Progression And Hybrid Journaling</h2><div class="diagram"><div class="mermaid">${diagrams.progressionLoop}</div></div><p>The progression layer now follows the breeding-gated ladder. Friendly starts unlocked, same-type child success unlocks the next type in order, each unlock seeds a new male and female into every zone, and wild butterflies shift into mixed-lineage breeding before eventually leaving after three successful hybrid pairings. The journal still stores hybrid lineage entries and the next hybrid id so discoveries remain personal and persistent.</p>${renderKeyValueTable('Progression storage',[{ key:'unlockedButterflyTypes', value:'currently unlocked base types in canonical order' },{ key:'unlockHistory', value:'ordered unlock events with source type and timestamp' },{ key:'starterPairsSeeded', value:'per-zone starter pair seeding truth' },{ key:'perTypeUnlockStatus', value:'per-type same-type completion and unlock state' },{ key:'perWildButterflyProgress', value:'per-wild butterfly same-type and hybrid progression' },{ key:'hybridJournal', value:'ordered hybrid entry list' },{ key:'nextHybridId', value:'next journal id counter' }])}</section>
