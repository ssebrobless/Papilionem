...
const OUTPUT_ARTIFACT_PATH = path.join(ROOT, 'assets', 'ml', 'm6-garden-policy.json');
const OUTPUT_ROOT = path.join(ROOT, 'qa_screenshots', 'm6_policy_training');
...
function buildCandidateArtifact(baseArtifact, records, metadata = {}) {
  const candidate = {
    ...baseArtifact,
    modelVersionId: 'm6-garden-policy-v1',
    trainedFrom: 'aa4-c2-trace-corpus-heldout-ridge-linear-v1',
    training: {
      trainer: 'scripts/train-m6-garden-policy.js',
...
function summarizeHeldOutEvaluations(evaluations = []) {
  ...
  for (const policyName of POLICY_FAMILIES) {
    ...
    perPolicy[policyName] = {
      ...
      m6RateMean: Number(artifactRateMean.toFixed(3)),
      m6RateStd: Number(std('artifactRate', artifactRateMean).toFixed(3)),
...
function evaluateHeldOut(baseArtifact = {}, records = []) {
  ...
  for (const seed of HOLDOUT_SEEDS) {
    ...
    const m6Eval = evaluateArtifactAgainstCorpus(candidate, split.holdout);
    const m4Eval = evaluateArtifactAgainstCorpus(baseArtifact, split.holdout);
    const perPolicy = {};
    for (const policyName of POLICY_FAMILIES) {
      perPolicy[policyName] = {
        total: m6Eval.perPolicy[policyName]?.total || 0,
        artifactRate: m6Eval.perPolicy[policyName]?.artifactRate || 0,
        heuristicRate: m6Eval.perPolicy[policyName]?.heuristicRate || 0,
        m4Rate: m4Eval.perPolicy[policyName]?.artifactRate || 0,
        artifactMatches: m6Eval.perPolicy[policyName]?.artifactMatches || 0,
        heuristicMatches: m6Eval.perPolicy[policyName]?.heuristicMatches || 0,
  ...
  return {
    ...
    promotionGate: {
      ...
      beatsM4PolicyCount,
      beatsM4AtLeastThree: beatsM4PolicyCount >= 3,
      ...
      autobattlePostureAtLeastPointSeven: (perPolicySummary.autobattlePosture?.m6RateMean || 0) >= 0.7
    }
  };
}
...
async function main() {
  ...
  const corpus = await buildC2TraceCorpus({
    ...
    sourceAudit: 'train-m6-garden-policy'
  });
  ...
  const m6 = buildCandidateArtifact(baseArtifact, records, {
  ...
  fs.writeFileSync(OUTPUT_ARTIFACT_PATH, JSON.stringify(m6, null, 2));
  const evaluation = {
    generatedAt: new Date().toISOString(),
    ...
    m6: evaluateArtifactAgainstCorpus(m6, records)
  };
  evaluation.promotionGate = {
    ...
    beatsM4AtLeastThreeHeldOut: heldOutEvaluation.promotionGate.beatsM4AtLeastThree,
    ...
    valueMetricsRequired: 'run scripts/run-ml-on-off-capture-audit.js --policy assets/ml/m6-garden-policy.json before default promotion',
  ...
  evaluation.promotionGate.artifactGatePass =
    ...
    && evaluation.promotionGate.noHeuristicRegressionHeldOut
    && evaluation.promotionGate.beatsM4AtLeastThreeHeldOut
    && evaluation.promotionGate.noM4RegressionWorseThanFivePercent
    && evaluation.promotionGate.autobattlePostureAtLeastPointSeven;
  const evaluationPath = path.join(outputDir, 'm6-training-evaluation.json');
