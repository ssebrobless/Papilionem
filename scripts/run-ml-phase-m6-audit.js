...
const OUTPUT_ROOT = path.join(ROOT, 'qa_screenshots', 'ml_phase_m6_audit');
...
function resolvePolicyArtifactPath(policyArg = POLICY_ARG) {
  if (!policyArg) return path.join(ROOT, 'assets', 'ml', 'm6-garden-policy.json');
  ...
  try {
    ...
    await phase(page, report, outputDir, '02-corpus-coverage-and-eval-thresholds', async () => {
      const corpus = await buildC2TraceCorpus({
        ...
        sourceAudit: 'run-ml-phase-m6-audit'
      });
    ...
    await phase(page, report, outputDir, '04-live-battle-ui-and-rounds', async () => {
      ...
      return {
        pass:
          ...
          state?.runtime?.artifactSummary?.modelVersionId === 'm6-garden-policy-v1',
        details: state
      };
    });
