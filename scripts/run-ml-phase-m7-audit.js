...
const OUTPUT_ROOT = path.join(ROOT, 'qa_screenshots', 'ml_phase_m7_audit');
  ...
  try {
    ...
    await phase(page, report, outputDir, '02-corpus-coverage-and-eval-thresholds', async () => {
      const corpus = await buildC2TraceCorpus({
        ...
        sourceAudit: 'run-ml-phase-m7-audit',
        balance: true
      });
      ...
      const heldOutGate = artifact?.training?.heldOutEvaluation?.promotionGate || {};
      const b6HeldOutGatePass =
        heldOutGate.protectedPolicyNoHeuristicRegression === true &&
        heldOutGate.beatsM4AtLeastFour === true &&
        heldOutGate.noM4RegressionWorseThanFivePercent === true &&
        heldOutGate.autobattlePostureAtLeastPointSeven === true;
      const coveragePass =
      ...
      return {
        pass: coveragePass && b6HeldOutGatePass === true,
        details: {
          manifestPath: corpus.manifestPath,
          recordsPath: corpus.recordsPath,
          evaluationPath,
          ...
          b6HeldOutGate: {
            source: 'artifact.training.heldOutEvaluation.promotionGate',
            pass: b6HeldOutGatePass,
            gate: heldOutGate,
            perPolicy: artifact?.training?.heldOutEvaluation?.perPolicy || null
          }
        }
      };
    });
